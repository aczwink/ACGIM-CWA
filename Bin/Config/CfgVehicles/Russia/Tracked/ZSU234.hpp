...
class Russia_ZSU234 : Tank
{
	...
	class EventHandlers
	{
		Init = "[_this select 0] exec {\acgim_scripts\Vehicles\ZSU234\podw.sqs}; [_this select 0] exec {\acgim_scripts\Vehicles\ZSU234\number.sqs}; [_this select 0] exec {\acgim_scripts\Vehicles\ZSU234\radar.sqs}";
		Fired="_this call loadFile {\acgim_scripts\Vehicles\ZSU234\stvol.sqf}";
		Killed="[_this select 0] exec {\acgim_scripts\Vehicles\ZSU234\destroyed.sqs}; (_this select 0) exec {\acgim_scripts\nmod_effects\DKMM_RSC_Veh_burner.sqs}";
		GetIn="if (((_this select 1) == ""gunner"") and !(player in (_this select 0))) then {[_this select 0] exec ""\acgim_scripts\Vehicles\ZSU234\radar.sqs""}";
