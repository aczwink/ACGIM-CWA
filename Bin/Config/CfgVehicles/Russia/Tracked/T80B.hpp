...
class T80 : RussianTank
{
	...
	class EventHandlers
	{
		init= "_this exec {\acgim_scripts\Vehicles\T80\RAE_T80_Init.sqs}";
		hit = "if (_this select 0 call RHS_countedCargo > 0) then {_this select 0 exec ""\acgim_scripts\Vehicles\T80\RAE_T80_throwOff.sqs""}; If(RHS_FctLoad)Then{_this exec ""\acgim_scripts\Vehicles\T80\ArmorHit.sqs""}";
		incomingMissile = "if (_this select 0 call RHS_countedCargo > 0) then {_this select 0 exec ""\acgim_scripts\Vehicles\T80\RAE_T80_throwOff.sqs""}";
		fired = "if (_this select 0 call RHS_countedCargo > 0) then {_this select 0 exec ""\acgim_scripts\Vehicles\T80\RAE_T80_throwOff.sqs""}; If(RHS_FctLoad && RHS_T80_FctLoad)Then{_this call RHS_TankFired; (_this + [_this call RHS_Noid]) call RHS_T80_Fired}";
