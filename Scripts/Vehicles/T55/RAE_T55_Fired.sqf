...
if(_w=="RHS_smokeDischarger")then{if(typeOf _t in["SIG_T55AM2","SIG_T55AMV","SIG_T55AM2w"])then{_n=8;_lw=120;_nw=15;};[_t,_n,_lw,_nw,40,true,40,false]exec "\RHS_Misc\armour\smoke\MAP_fireSmoke.sqs";}else{
if (RHS_lowSmoke) then {_a=0.5};
if (_m in RHS_CannonAmmo) then {_this exec format ["%1RAE_T55_RecoilBarrel.sqs", RHS_PathT55]; _this exec format ["%1RAE_T55_RecoilTank.sqs", RHS_PathT55];
_d=direction _t;
_dT=_d;_ra=2;
